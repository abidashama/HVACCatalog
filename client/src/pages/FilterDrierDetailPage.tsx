...
const toTitleCase = (str: string): string => {
  ...
  const acronyms = ['hvac', 'ac', 'dc', 'ul', 'ce', 'sek', 'spl']
...
export default function FilterDrierDetailPage() {
  const [, params] = useRoute('/filter-driers/:subcategoryId')
  ...
  const categories = filterDrierData.categories as Record<string, SubcategoryData>

  // Map subcategory IDs to data
  const subcategoryMap: Record<string, SubcategoryData> = {
    'filter-driers': categories.filterDriers,
    'filter-drier-shell': categories.filterDrierShell
  }
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex justify-center mt-6">
            <Button onClick={() => setLocation(FILTER_DRIERS_URL)}>
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        <Button
          variant="ghost"
          onClick={() => setLocation(FILTER_DRIERS_URL)}
          ...
          <div className="space-y-8" ref={productInfoRef}>
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-3 py-1 text-sm">Filter Driers/Filter Drier Shell</Badge>
              </div>
              ...
              <p className="text-slate-600 leading-relaxed mb-8 text-lg">
                High-quality {toTitleCase(currentSubcategory.name).toLowerCase()} designed for industrial HVAC and refrigeration applications. 
                ...
                <Table className="premium-specs-table">
                  <TableHeader>
                    <TableRow>
                      ...
                      {currentSubcategory.products.some((p) => p.height) && (
                        <TableHead className="specs-table-header">Height</TableHead>
                      )}
                      {currentSubcategory.products.some((p) => p.core) && (
                        <TableHead className="specs-table-header">Core Type</TableHead>
                      )}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {currentSubcategory.products.map((product, index) => (
                      <TableRow key={index} className="specs-table-row" style={{'--row-index': index} as React.CSSProperties}>
                        ...
                        <TableCell className="specs-table-cell">
                          <span className="font-mono text-sm">{product.connection}</span>
                        </TableCell>
                        {currentSubcategory.products.some((p) => p.height) && (
                          <TableCell className="specs-table-cell">
                            {product.height ? <span className="font-mono text-sm">{product.height}</span> : '-'}
                          </TableCell>
                        )}
                        {currentSubcategory.products.some((p) => p.core) && (
                          <TableCell className="specs-table-cell">
                            {product.core ? (
                              <Badge variant={product.core.includes('double') ? 'default' : 'secondary'}>
                                {product.core}
                              </Badge>
                            ) : '-'}
                          </TableCell>
