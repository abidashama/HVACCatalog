...
const toTitleCase = (str: string): string => {
  ...
  const acronyms = ['hvac', 'ac', 'dc', 'ul', 'ce']
...
export default function TeflonTapeDetailPage() {
  const [, params] = useRoute('/teflon-tape/:subcategoryId')
  ...
  const categories = teflonTapeData.categories as Record<string, SubcategoryData>

  // Map subcategory IDs to data
  const subcategoryMap: Record<string, SubcategoryData> = {
    'teflon-tape': categories.teflonTape
  }
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex justify-center mt-6">
            <Button onClick={() => setLocation(TEFLON_TAPE_URL)}>
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        <Button
          variant="ghost"
          onClick={() => setLocation(TEFLON_TAPE_URL)}
          ...
          <div className="space-y-8" ref={productInfoRef}>
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-3 py-1 text-sm">Teflon Tape</Badge>
              </div>
                ...
                <Table className="premium-specs-table">
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="specs-table-header">Weight</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {currentSubcategory.products.map((product, index) => (
                      <TableRow key={index} className="specs-table-row" style={{'--row-index': index} as React.CSSProperties}>
                        ...
                        <TableCell className="specs-table-cell">
                          <span className="font-mono text-sm">{product.weight}</span>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
