...
const FANS_URL = '/products?category=Axial%20Fans%2FShaded%20Pole%20Motor%2FSmall%20Fans'
...
const toTitleCase = (str: string): string => {
  ...
  const acronyms = ['lp', 'hp', 'hvac', 'ac', 'dc', 'ul', 'ce', 'nc', 'no', 'yc', 'cfm', 'rpm']
...
export default function FansDetailPage() {
  const [, params] = useRoute('/fans/:productId')
  ...
  const productId = params?.productId || 'fans-axialFans-singlePhaseSuction'
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex justify-center mt-6">
            <Button onClick={() => setLocation(FANS_URL)}>
      ...
      <div className="relative z-10 py-6 px-4">
        <div className="max-w-7xl mx-auto">
          <NavigationBreadcrumb 
            items={[
              { label: 'Home', href: '/' },
              { label: 'Axial Fans/Shaded Pole Motor/Small Fans', href: FANS_URL },
              { label: toTitleCase(product.title), href: `/fans/${product.id}` }
            ]}
          />
        </div>
      </div>
          ...
          <div className="space-y-8" ref={productInfoRef}>
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-3 py-1 text-sm">Axial Fans/Shaded Pole Motor/Small Fans</Badge>
              </div>
              ...
              <p className="text-slate-600 leading-relaxed mb-8 text-lg">
                High-performance {toTitleCase(product.title).toLowerCase()} designed for industrial HVAC and refrigeration ventilation applications. 
                      ...
                      <Table className="premium-specs-table">
                        <TableHeader>
                          <TableRow>
                            ...
                            {categoryModels.some((m) => m.size) && (
                              <TableHead className="specs-table-header">Size</TableHead>
                            )}
                            {categoryModels.some((m) => m.speed) && (
                              <TableHead className="specs-table-header">Speed</TableHead>
                            )}
                            {categoryModels.some((m) => m.airFlow) && (
                              <TableHead className="specs-table-header">Air Flow</TableHead>
                            )}
                            {categoryModels.some((m) => m.power) && (
                              <TableHead className="specs-table-header">Power</TableHead>
                            )}
                            {categoryModels.some((m) => m.current) && (
                              <TableHead className="specs-table-header">Current</TableHead>
                            )}
                            {categoryModels.some((m) => m.capacitor) && (
                              <TableHead className="specs-table-header">Capacitor</TableHead>
                            )}
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                            ...
                            <TableRow key={`${categoryName}-${index}`} className="specs-table-row" style={{'--row-index': index} as React.CSSProperties}>
                              ...
                              {categoryModels.some((m) => m.size) && (
                                <TableCell className="specs-table-cell">
                                  {model.size ? `${model.size} ${model.sizeUnit || ''}` : '—'}
                                </TableCell>
                              )}
                              {categoryModels.some((m) => m.speed) && (
                                <TableCell className="specs-table-cell">
                                  {model.speed ? `${model.speed} ${model.speedUnit || ''}` : '—'}
                                </TableCell>
                              )}
                              {categoryModels.some((m) => m.airFlow) && (
                                <TableCell className="specs-table-cell">
                                  {model.airFlow ? `${model.airFlow} ${model.airFlowUnit || ''}` : '—'}
                                </TableCell>
                              )}
                              {categoryModels.some((m) => m.power) && (
                                <TableCell className="specs-table-cell">{model.power || '—'}</TableCell>
                              )}
                              {categoryModels.some((m) => m.current) && (
                                <TableCell className="specs-table-cell">{model.current || '—'}</TableCell>
                              )}
                              {categoryModels.some((m) => m.capacitor) && (
                                <TableCell className="specs-table-cell">{model.capacitor || '—'}</TableCell>
                              )}
                            </TableRow>
                          ))}
                        </TableBody>
